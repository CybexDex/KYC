...
class q:
    ...
    def _write(self, x, message):
        ...
        if t==1:
            message.fromstring(struct.pack('b', 10))
        else:
            message.fromstring(struct.pack('b', t))
        writeType = {
            -1: self._wb,
    ...
    def k3(self, query, args=None):
        global SYNC
        if isinstance(query, str) and args is None: 
            query = bytes(query,'utf8')
            self._send(SYNC, array.array('b',query))
        else:
            stuff = [array.array('b',query),]
            for item in args:
                stuff.append(item)
    ...
    def _rd(self, little_endian, bytearray):
        ...
        return datetime.date.fromtimestamp(self.gl(946684800)) + delta  #946684800L is conversion from UNIX epoch to KDB epoch
    ...
    def _rdt(self, little_endian, bytearray):
        ...
        return datetime.datetime.fromtimestamp(self.gl(946684800)) + delta  #946684800L is conversion from UNIX epoch to KDB epoch
    ...
    def _r(self, little_endian, bytearray):
        ...
        if t < 0 :
            ...
            if readType.get(t) is not None : return readType[t]()
        if t > 99 :
            if t == 100 :
